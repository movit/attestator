...
import com.attestator.admin.client.ui.event.GridGroupClickEvent.HasGridGroupClickHandlers;
 ...
 */
public class GroupingViewExt<M> extends GridView<M> implements HasCollapseItemHandlers<List<M>>,
    HasExpandItemHandlers<List<M>>, HasGridGroupClickHandlers<XElement> {
  ...
  public static interface GroupingViewExtCss extends CssResource {
      String groupingViewExtLink();
  }  
  ...
  private final ClicableGroupingViewAppearance groupAppearance;
  ...
  public GroupingViewExt() {
    this(GWT.<GridAppearance> create(GridAppearance.class), GWT.<ClicableGroupingViewAppearance> create(ClicableGroupingViewAppearance.class));
  }
   ...
   */
  public GroupingViewExt(GridAppearance appearance, ClicableGroupingViewAppearance groupingAppearance) {
    super(appearance);
   ...
   */
  public GroupingViewExt(ClicableGroupingViewAppearance groupAppearance) {
  ...
  protected SafeHtml doRender(List<ColumnData> cs, List<M> rows, int startRow) {
    ...
    for (int j = 0; j < rows.size(); j++) {
      ...
      if (curGroup == null || !isModelsEquals(groupingColumn, curGroup.getValue(), gvalue)) {
   ...
   */
  public ClicableGroupingViewAppearance getGroupingAppearance() {
  ...
  private <N> boolean isModelsEquals(ColumnConfig<?, N> column, Object o1, Object o2) {
      if (column.getComparator() != null) {
          return column.getComparator().compare((N)o1, (N)o2) == 0;
      }
          ...
          return Util.equalWithNull(o1, o2);
      }
  }

    @Override
    public HandlerRegistration addGridGroupClickHandler(
            GridGroupClickHandler<XElement> handler) {
        return handlerManager.addHandler(GridGroupClickEvent.getType(), handler);
    }
