...
public class GroupSummaryViewExt<M> extends GroupingView<M> {

    public GroupSummaryViewExt() {
        super();
    }
    ...
    protected SafeHtml renderSummary(GroupingData<M> groupInfo, Map<ValueProvider<? super M, ?>, Number> data) {
      ...
      for (int i = 0, len = colCount; i < len; i++) {
        ...
        SafeHtml tdContent = tpls.td(i, cellClasses, cd.getStyles(), cellInner, cf.getColumnTextStyle(), SafeHtmlUtils.fromTrustedString(value));
